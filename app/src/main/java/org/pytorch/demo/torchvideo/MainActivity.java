...
public class MainActivity extends AppCompatActivity implements Runnable {
    ...
    private final String[] mTestVideos = {"video1", "video2", "video3", "video4", "video5", "video6", "video7", "video8"};
...
//        mButtonTest.setText(String.format("Test 1/%d", mTestVideos.length));
        mButtonTest.setText("Random Sample");
...
//                    mButtonPauseResume.setText(getString(R.string.resume));
                    mButtonPauseResume.setText("Start");
                    mVideoView.pause();
                }
                else {
//                    mButtonPauseResume.setText(getString(R.string.pause));
                    mButtonPauseResume.setText("Stop");
                    mVideoView.start();
                }
            }
        });
    ...
    public void run() {
        ...
        for (int i = 0; !mStopThread && i < durationTo; i++) {
            ...
            runOnUiThread(new Runnable() {
                ...
                public void run() {
                    ...
                    String show = String.format("<b>Timestamp:</b> %ds <br> <b>Predictions:</b> %s <br> <b>Latency:</b> %dms", finalI +1, result, inferenceTime);
                    mTextView.setText(Html.fromHtml(show));
                }
            });
            mResults.add(result);
        }
        ...
        if (!mStopThread) {
            runOnUiThread(() -> mButtonPauseResume.setText("Start"));
